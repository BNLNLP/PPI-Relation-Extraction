...
# [GP][START] - preprocess BioCreative data. 11-18-2020
def remove_unnecessary_token(psg_data):
	...
	for tok_idx, tok in enumerate(psg_data['parsed_text']):
		if tok == '-':
			for x in range(len(psg_data['entities'])): # (ent_id, ent_text, ent_type, ent_start, ent_end, offset, length)
...
def preprocess_biocreative(data, nlp):
	"""
	to preprocess 'PMtask_Relations_TrainingSet.json' and 'PMtask_Relations_TestSet.json' for pretraining.
	"""
	D = []
...
def load_dataloaders(args, max_length=50000):
	...
	if not os.path.isfile("./data/D.pkl"):
		...
		# [GP][START] - preprocess BioCreative data. 11-18-2020
		if args.pretrain_data.endswith('PMtask_Relations_TrainingSet.json'):
			with open(args.pretrain_data) as fp:
				data = json.load(fp)
			
			nlp = spacy.load("en_core_web_lg", disable=["tagger", "ner"])

			D = preprocess_biocreative(data, nlp)
		else:
		# [GP][END] - preprocess BioCreative data. 11-18-2020
...
class pretrain_dataset(Dataset):
	...
	def tokenize(self, D):
		...
		'''
		print('x:', x)
		print('masked_for_pred:', masked_for_pred)
		#input('enter..')
		'''
		...
		#print('x ids:', x)
		#print('masked_for_pred ids:', masked_for_pred)
		
		
		
		
		'''
		x = [self.tokenizer.tokenize(t) for t in x]
		x = [j for sub in x for j in sub] 
		masked_for_pred = [self.tokenizer.tokenize(t) for t in masked_for_pred]
		masked_for_pred = [j for sub in masked_for_pred for j in sub]
		
		print('tokenized x:', x)
		print('tokenized masked_for_pred:', masked_for_pred)
		
		
		x = self.tokenizer.convert_tokens_to_ids(x)
		masked_for_pred = self.tokenizer.convert_tokens_to_ids(masked_for_pred)
		
		print('tokenized x ids:', x)
		print('tokenized masked_for_pred ids:', masked_for_pred)
