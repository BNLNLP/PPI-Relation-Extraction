...
def train_and_fit(args):
	...
	for epoch in range(start_epoch, args.num_epochs):
		...
		for i, data in enumerate(train_loader, 0):
			...
			'''
			print('data:', data)
			print('blank_labels:', blank_labels)
			print('x.shape:', x.shape)
			print('masked_for_pred.shape:', masked_for_pred.shape)
			print('e1_e2_start.shape:', e1_e2_start.shape)
			print('blank_labels.shape:', blank_labels.shape)
			#input('enter...')
			
			#if len([i for i, l in enumerate(blank_labels.squeeze().tolist()) if l == 1]) <= 1:
			#	continue
			'''
			...
			'''
			print('blanks_logits:', blanks_logits)
			print('blanks_logits.shape:', blanks_logits.shape)
			print('lm_logits:', lm_logits)
			print('masked_for_pred:', masked_for_pred)
			print('lm_logits.shape:', lm_logits.shape)
			print('masked_for_pred.shape:', masked_for_pred.shape)
