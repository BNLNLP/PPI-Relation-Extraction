"""
This generates 10 fold cross-validation sets for PPI datasets. 
...
data_file_dict = {# Original datasets (labels: positive, negative)
                  #"AImed": "datasets/PPI/original/AImed/AImed.xml",
                  "AImed": "datasets/PPI/original/AImed/AImed_updated.xml", # Four sentences were modified to fix annotaion errors.
                  "BioInfer": "datasets/PPI/original/BioInfer/BioInferCLAnalysis_split_SMBM_version.xml",
                  "HPRD50": "datasets/PPI/original/HPRD50/HPRD50.xml",
                  "IEPA": "datasets/PPI/original/IEPA/IEPA.xml",
                  "LLL": "datasets/PPI/original/LLL/LLL.xml",
                  
                  # Type annotated datasets (labels: enzyme, structural, negative) - 1st: positive (enzyme, structurel), 2nd: negative
                  # Negative annotations - WARNING!! DO NOT forget to set 'neg_sample_processing = True' !!
                  "AImed_typed": ["aimed_type_annotations_srm.tsv", "passed_full_aimed_training_ikb.tsv"],
                  "BioInfer_typed": ["bioinfer_type_annotations_struct_histones_srm.tsv", "passed_full_bioinfer_training.tsv"],
                  "HPRD50_typed": ["hprd50_type_annotation_srm.tsv", "passed_full_hprd50_training.tsv"],
                  "IEPA_typed": ["iepa_type_annotations_srm.tsv", "passed_full_iepa_training.tsv"],
                  "LLL_typed": ["lll_type_annotations_srm.tsv", None],
                  ...
                  #export DATA1=~/BER-NLP/RE/datasets/PPI/type_annotation/AImed_type/aimed_type_annotations_srm.filtered.tsv
                  #export DATA1=~/BER-NLP/RE/datasets/PPI/type_annotation/BioInfer_type/bioinfer_type_annotations_struct_histones_srm.filtered.tsv
                  #export DATA1=~/BER-NLP/RE/datasets/PPI/type_annotation/HPRD50_type/hprd50_type_annotation_srm.filtered.tsv
                  #export DATA1=~/BER-NLP/RE/datasets/PPI/type_annotation/IEPA_type/iepa_type_annotations_srm.filtered.tsv
                  #export DATA1=~/BER-NLP/RE/datasets/PPI/type_annotation/LLL_type/lll_type_annotations_srm.filtered.tsv
                  
                  "BioCreative_type": "01_27_2021/IKB_Sean_BioCreative_full_text_annotation.txt",
                  }
        ...
        for rel in relation:
            ...
            for e1 in entity_grp_1:
                for e2 in entity_grp_2:
                    ...
                    # TODO: handle the cases where entities overlap.
                    ...
                    if (e1_s_idx > e2_s_idx and e1_s_idx < e2_e_idx) or (e2_s_idx > e1_s_idx and e2_s_idx < e1_e_idx):
                        logger.info('entities overlap: %s', tagged_sent)
                        continue
            ...
            for psg_data in psg_data_list:
                ...
                for g1e in gene1_entities:
                    for g2e in gene2_entities:
                        ...
                        # TODO: handle self PPIs and cases where entities overlap.
...
def create_entity_type_file(file):
    """
    this creates 'entity_types.json' file.
    ...
    ## TODO: fix the path for type_annotation.
    if 'type_annotation' in file:
        entity_file = 'datasets/ppi/type_annotation/entity_types.json'
        unique_entity_types['gene/protein'] = {'id': len(unique_entity_types)} # when processing Typed PPI, set BioCreative and HPRD50 entities as gene/protein. 01-08-2022
    else:
        entity_file = os.path.join(file.rsplit('/', 1)[0], 'entity_types.json')
...
def get_samples_from_ppi_benchmark(file):
    ...
    entity_type_file = os.path.join(file.rsplit('/', 1)[0], 'entity_types.json')
    if not os.path.exists(entity_type_file):
        create_entity_type_file(file)
    ...
    # debug
    num_of_separate_ent = 0
    num_of_overlapping_ent = 0
    ...
    for doc_elem in root.findall('.//document'):
        ...
        for sent_elem in doc_elem.findall('.//sentence'):
            ...
            for ent_elem in sent_elem.findall('.//entity'):
                ...
                # Entities may consist of separate words in a sentence.
                # So far, only BioInfer has such examples.
                # e.g., "65-70,82-86" -> 'muscle actin' in "...muscle and brain actin ..." in BioInfer
                ent_char_offset_list = ent_char_offset.split(',')

                offset_list = []
                for ent_char_offset in ent_char_offset_list:
                    offset_s = int(ent_char_offset.split('-')[0])
                    offset_e = int(ent_char_offset.split('-')[1]) + 1
                    offset_list.append((offset_s, offset_e))
                
                entities[ent_id] = {'offset_list': offset_list, 'text': ent_text, 'type': ent_type}
                
                # debug
                if bool(ent_text == ' '.join([sent_txt[s:e] for (s, e) in offset_list])) == False:
                    print('entity text from file:', ent_text)
                    print('entity text by  index:', ' '.join([sent_txt[s:e] for (s, e) in offset_list]))
                    input('enter..')
            ...
            for pair_elem in sent_elem.findall('.//pair'):
                ...
                # TODO: Need to handle overlapping and separate entities for entity marker representation later.
                # Fow now, only non-overlapping and non-separate entities with samples are used for entity marker representation.    
                # For entity mention representation, it's not a problem, but it's complicated for entity marker representation.
                # E.g., AImed.xml: 
                ...
                # For now, just use the first offset pair for the following debug and garbage values for entity marker text.
                e1_start_idx, e1_end_idx = e1_offset_list[0]
                e2_start_idx, e2_end_idx = e2_offset_list[0]
                ...
                if e1_start_idx == e2_start_idx or \
                   ...
                   len(e1_offset_list) > 1 or len(e2_offset_list) > 1:

                    # Caution! Don't use 'None' for these variables. 
                    # It may cause the error in Apache arrow array -> Error msg: "Check failed: (off) <= (length) Slice offset greater than array length"
                    text_with_entity_marker = sent_txt
                    text_with_typed_entity_marker = sent_txt
                    e1_s_in_text_with_entity_marker = -1
                    e1_e_in_text_with_entity_marker = -1
                    e2_s_in_text_with_entity_marker = -1
                    e2_e_in_text_with_entity_marker = -1
                    e1_s_in_text_with_typed_entity_marker = -1
    ...
    # debug
    print('>> num_of_separate_ent:', num_of_separate_ent)
    print('>> num_of_overlapping_ent:', num_of_overlapping_ent)
...
def store_doc_data(train, dev, test, fold_num, output_dir):
    """
...
def main():
    ...
    if dataset_name in ["AImed", "BioInfer", "HPRD50", "IEPA", "LLL"]:
        ...
        pos_data_file = os.path.join(current_working_dir, pos_data_file)
        doc_samples = get_samples_from_ppi_benchmark_type(pos_data_file, neg_sample_processing=False)
        output_dir = pos_data_file.rsplit('/', 1)[0] + '/10-fold-cv_pos'
        split_and_save(doc_samples, output_dir)
        if neg_data is not None:
            neg_data_file = os.path.join(current_working_dir, neg_data_file)
            doc_samples = get_samples_from_ppi_benchmark_type(neg_data_file, neg_sample_processing=True)
            output_dir = neg_data_file.rsplit('/', 1)[0] + '/10-fold-cv_neg'
            split_and_save(doc_samples, output_dir)
    elif dataset_name == "BioCreative_type":
        pos_data = data_file_dict[dataset_name]
        pos_data_file = os.path.join(current_working_dir, pos_data_file)
        doc_samples = get_samples_from_biocreative_type(pos_data_file)
        output_dir = pos_data_file.rsplit('/', 1)[0] + '/10-fold-cv_pos'
        split_and_save(doc_samples, output_dir)
    # (not used for now)
    #elif dataset_name == "BioCreative":
    #	data = data_file_dict[dataset_name]
    #	doc_samples = get_samples_from_biocreative(data)
    else:
